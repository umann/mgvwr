...
static bool validate_against_with_path(const SchemaResolver &resolver, const json &schema, const json &instance,
    ...
    if (instance.is_object() && resolvedSchema.contains("properties")) {
        ...
        for (const auto &[key, propSchema] : props.items()) {
            if (instance.contains(key)) {
                if (!validate_against_with_path(resolver, resolver.resolve(propSchema), instance[key],
                                                path + "." + key, suppressErrors)) {
                    return false;
                }
            }
        }
...
static std::optional<json> enrich_object_missing(const SchemaResolver &resolver, const json &schema) {
    ...
    for (const auto &[key, propSchema] : props.items()) {
        const auto child = [&](const json &s) {
            const auto childOpt = [&]() -> std::optional<json> {
                ...
                if (s.contains("allOf") && s["allOf"].is_array()) {
                    ...
                    bool found = false;
                    for (const auto &sub : s["allOf"]) {
                        const auto subOpt = [&]() -> std::optional<json> {
                            if (sub.is_object() && sub.contains("default")) {
                                return sub["default"];
                            }
                            return std::nullopt;
                        }();
    ...
    return any ? std::optional<json>(result) : std::nullopt;
}
...
static bool is_object_schema(const json &schema) {
    ...
    if (schema.contains("type")) {
        const auto &typeConstraint = schema["type"];
        if (typeConstraint.is_string()) {
            return typeConstraint.get<std::string>() == "object";
        }
        if (typeConstraint.is_array()) {
            for (const auto &entry : typeConstraint) {
                if (entry.is_string() && entry.get<std::string>() == "object") {
                    return true;
                }
            }
        }
...
static bool is_array_schema(const json &schema) {
    if (!schema.is_object()) {
        return false;
    }
    if (schema.contains("type")) {
        const auto &typeConstraint = schema["type"];
        if (typeConstraint.is_string()) {
            return typeConstraint.get<std::string>() == "array";
        }
        if (typeConstraint.is_array()) {
            for (const auto &entry : typeConstraint) {
...
static std::optional<json> enrich(const SchemaResolver &resolver, const json &schema0, std::optional<json> instance) {
    json schema = resolver.resolve(schema0);

    if (schema.contains("allOf") && schema["allOf"].is_array()) {
        std::optional<json> current = instance;
        for (const auto &subSchema : schema["allOf"]) {
            current = enrich(resolver, subSchema, current);
        }
        return current;
    }
    ...
    if (schema.contains("oneOf") && schema["oneOf"].is_array()) {
        if (!instance.has_value()) {
            if (schema.contains("default")) {
                return enrich(resolver, schema, schema["default"]);
            }
            return std::nullopt;
        }
        ...
        for (const auto &branch : schema["oneOf"]) {
            auto candidate = enrich(resolver, branch, *instance);
            if (!candidate.has_value()) {
                continue;
            }
            if (validate_against_with_path(resolver, branch, *candidate, "$", true)) {
    ...
    if (!instance.has_value()) {
        if (schema.contains("default")) {
            json defaultValue = schema["default"];
            return enrich(resolver, schema, defaultValue);
        }
        if (is_object_schema(schema)) {
            return enrich_object_missing(resolver, schema);
        }
        return std::nullopt;
    }

    json current = *instance;
    if (is_object_schema(schema) && current.is_object()) {
        const json &props = schema["properties"];
        for (const auto &[key, propSchema] : props.items()) {
            if (current.contains(key) && current[key].is_null()) {
                auto child = enrich(resolver, propSchema, std::nullopt);
                if (child.has_value()) {
                    current[key] = *child;
                }
            } else if (current.contains(key)) {
                auto child = enrich(resolver, propSchema, current[key]);
                if (child.has_value()) {
                    current[key] = *child;
                }
            } else {
    ...
    if (is_array_schema(schema) && current.is_array() && schema.contains("items")) {
        for (auto &element : current) {
            auto child = enrich(resolver, schema["items"], element);
            if (child.has_value()) {
                element = *child;
            }
        }
