...
class MapViewer {
  public:
    MapViewer(const std::string &cacheDir, size_t maxCacheSizeMB, int windowWidth = 1024, int windowHeight = 768,
              bool inlineMode = false, int minZoom = 2, int maxZoom = 18);
    ...
    bool isLoadingTiles() const {
        return isDownloadingTiles || (totalVisibleTiles > 0 && tilesLoaded < totalVisibleTiles);
    }
    ...
    // Tile coordinate structure (public for cache functionality)
    struct TileCoord {
        int x, y, zoom;
